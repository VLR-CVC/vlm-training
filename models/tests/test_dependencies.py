...
both are legitimate configurations -- JUPITER is ARM and has no `flash_attn`.
...
REQUIRED = {
    ...
    "qwen4": [("transformers", ()), ("torch", ())] + FLA_MODULES,
}
...
OPTIONAL = {
    ...
    # `causal_conv1d_torch` stands in, at a real cost.
    "qwen3_5": CONV1D_MODULES,
    # ...plus flash_qla for the QSA path, which is SM120-only today.
    "qwen4": CONV1D_MODULES + [("flash_qla", ("chunk_gated_delta_rule",))],
}
...
@pytest.mark.parametrize("family", ["qwen3_5", "qwen4"])
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("transformers.models.qwen4_exp", "qwen4"),
    ],
)
def test_transformers_reference_architecture(module, family):
    ...
    `qwen4_exp` first ships in transformers 5.16.0, so a checkout pinned to
    5.6.0 skips it.
    """
