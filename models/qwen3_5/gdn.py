# Vendored from torchtitan b21f7d43e: torchtitan/models/qwen3_5/gdn.py
...
#
# Kernels are attn_gym.linear (attn-gym[linear]==0.0.8). Its GDN fake kernels need
# scripts/scaling/ttbench/patches/attn_gym_gdn_fake_strides.py applied, or compiled
# graphs fail a stride assert tens of steps in (TORCHTITAN_BENCHMARK.md phase 4).
...
from train.parallel.batch_invariant import is_in_batch_invariant_mode
from models.common.nn_modules import Conv1d, Linear
from models.common.attention import VarlenMetadata
...
class GatedDeltaKernel(Module):
    ...
    def forward(
        ...
        *,
        ...
        # DEVIATION from torchtitan b21f7d43e: the fused chunk kernel accepts fp16/bf16
        # Q/K/V only. With fp32 compute (e.g. parity tests that remove bf16 GEMM
        # rounding) run the kernel in bf16 and return the caller's dtype.
        out_dtype = xq_THK.dtype
        if out_dtype not in (torch.float16, torch.bfloat16):
            xq_THK, xk_THK, xv_THV = (t.to(torch.bfloat16) for t in (xq_THK, xk_THK, xv_THV))
...
class InnerGatedDeltaNet(Module):
    ...
    def forward(
        ...
        *,
        ...
        # DEVIATION from torchtitan b21f7d43e, which takes the dense (non-varlen)
