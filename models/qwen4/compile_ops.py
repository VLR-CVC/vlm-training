...
try:
    ...
    # low-level split of rms_norm_gated (fla)
...
try:
    ...
    _causal_conv1d_fn = _conv_fwd_fn = _conv_bwd_fn = None


def causal_conv1d_available() -> bool:
    """Whether the fused `causal_conv1d` CUDA kernel is importable here."""
    return _causal_conv1d_fn is not None


def causal_conv1d_torch(x, weight, bias, seq_idx):
    """Plain-torch stand-in for `causal_conv1d_fn(..., activation="silu")`.

    Same contract as the kernel: `x` is (B, C, L), `weight` (C, K), `bias` (C,)
    or None, and `seq_idx` (B, L) the packed-document id of every token. Taps
    that reach back into the previous document are dropped, which is what the
    kernel's segmented mode does.

    Materializes the (B, C, L, K) tap window, so it costs `kernel_size` times
    the activation the fused kernel needs, and accumulates in fp32. This is the
    correctness path for CPU and for hosts without the extension, not a fast one.
    """
    K = weight.shape[-1]
    # fp32 accumulation like the kernel, without demoting an fp64 input
    acc = torch.promote_types(x.dtype, torch.float32)
    windows = F.pad(x.to(acc), (K - 1, 0)).unfold(-1, K, 1)  # (B, C, L, K)
    if seq_idx is not None:
        # -1 matches no real document id, so the left pad is always masked out
        ids = F.pad(seq_idx, (K - 1, 0), value=-1).unfold(-1, K, 1)  # (B, L, K)
        windows = windows * (ids == seq_idx.unsqueeze(-1)).unsqueeze(1)
    out = torch.einsum("bclk,ck->bcl", windows, weight.to(acc))
    if bias is not None:
        out = out + bias.to(acc).unsqueeze(-1)
